...
class CustomAttributesDropdown(base.Component):
  """A generic model for a dropdown in custom attributes."""
  _locator = locator.CustomAttributesDropdown

  def __init__(self, driver):
    super(CustomAttributesDropdown, self).__init__(driver)
  ...
  def add_new_custom_attribute(self, ca_object):
    ...
    if ca_object.mandatory is True:
  ...
  def _set_custom_attributes_list(self):
    """
    Set self.custom_attributes list with CustomAttribute objects from the
    current dropdown opened.
    """
